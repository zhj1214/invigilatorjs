  ...
  <view class="content-card" @click="toContentDetail">
    ...
    <view class="sp-content-center">
      ...
      <view class="text">
        ...
        <text>{{ contentInfo.detail.length > 60 ? calcDetail(contentInfo.detail) : contentInfo.detail }}</text>
      </view>
      <!-- 图片 -->
    ...
    <view v-if="showFooter" class="footer">
      ...
      <view class="right flex-row">
        ...
        <view class="like-btn" @click.stop="toLikeOrUnLike">
...
import { PropType, inject, reactive, computed, watch } from 'vue'
...
// 点赞图片 已点赞图片 评论图片
const [thumbUpIcon, thumbUpActiveIcon, commentIcon] = gdp(['icon-thumb-up.png', 'icon-thumb-up-active.png', 'icon-comment.png'])
...
const goCurrentGroup = (id: string) => {
  const { groupId, route } = currentPage as { groupId: string; route: string; [key: string]: any }
  if (route === 'package-community/pages/group-home/group-home' && groupId === id) {
    return
  }
  uni.navigateTo({
...
// 跳转话题详情页
const goToTopic = () => {
  const { topicId, route } = currentPage as { id: string; route: string; [key: string]: any }
  if (route === 'package-community/pages/topic-home/topic-home' && topicId === (props.info as ContentInfo).topicId) {
    return
  }
  uni.navigateTo({
...
async function updateFollow() {
  ...
  if (!follow) {
    ...
    emit('update-follow', authorId, true)
  }
}
...
async function updateContent(id: string) {
  const { like, likeCount } = (await getContentDetail(id)) as AnyObject
  contentInfo.likeCount = likeCount
  contentInfo.like = like
}

// 计算话题广场内容长度(处理表情符号)
function calcDetail(info: string) {
  let pointAt = Array.from(info)[58] as unknown as AnyObject
  let index = pointAt.codePointAt(0) > 0xffff ? 58 + 1 : 58
